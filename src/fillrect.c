...
void fillrect(const struct framedraw_info *info, const struct framedraw_fillrect *fillrect)
{
    ...
    if (!left) {
        ...
        switch (fillrect->rop) {
            case FRAMEDRAW_ROP_XOR:
                func = fillrect_xor_aligned;
                break;

            case FRAMEDRAW_ROP_COPY: default:
                func = fillrect_copy_aligned;
                break;
        }

        for (hcount = 0; hcount < fillrect->height; ++hcount) {
            base += index >> (BITS_PER_LONG - __builtin_clzl(BITS_PER_LONG));
